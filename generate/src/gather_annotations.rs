...
pub fn gather_annotations(
	...
	for (i, lang) in annotation_langs.clone().into_iter().enumerate() {
		println!("Processing annotations for language {}. Progress {}/{}", lang, i, langs_len);
		...
		for e in parser {
			match e {
				Ok(XmlEvent::StartElement { attributes, .. }) => {
					...
					if let Some(tts) = attributes.into_iter().find(|attr| attr.name.local_name == "type") {
				...
				Ok(XmlEvent::Characters(data)) => {
					if let Some(glyph) = current_glyph.clone() {
						match annotations.get_mut(&glyph) {
							Some(prev_entry) => {
								if let Some(same_lang) = prev_entry.iter_mut().find(|s| &s.lang == lang) {
									if is_tts {
									...
									prev_entry.push(if is_tts {
										Annotation::new(lang.to_string(), Some(data.trim().to_string()), "".to_owned())
									} else {
							...
							None => {
								if is_tts {
									...
									annotations.insert(glyph, vec![Annotation::new(lang.to_string(), None, data)]);
								}
							}
						};
					}
