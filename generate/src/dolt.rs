...
use sea_query::{Query, MysqlQueryBuilder, Order, Expr, Alias};
use sea_query::Iden;

use crate::{Annotation, Emoji, group_subgroup::Group, group_subgroup::Subgroup, emoji::Status, emoji::Version, emoji::SkinTone};
...
pub fn load_all() -> Result<(String, f32, Vec<Group>, HashMap<u32, String>, HashMap<u32, String>), Box<dyn Error>> {
    // 1. Fetch Groups
    ...
    let subgroup_map: HashMap<u32, String> = db_subgroups.iter().map(|s| (s.id, s.name.clone())).collect();

    // 3. Fetch Emojis
    let query = Query::select()
        .columns([Emojis::Hexcode, Emojis::Emoji, Emojis::Type, Emojis::Version, Emojis::GroupId, Emojis::SubgroupId, Emojis::Ordering, Emojis::ColorOrder])
        .from(Emojis::Table)
    ...
    // 4. Fetch Variants
    let query = Query::select()
        .columns([EmojiSkinToneVariants::Hexcode, EmojiSkinToneVariants::Emoji, EmojiSkinToneVariants::BaseHexcode, EmojiSkinToneVariants::Ordering, EmojiSkinToneVariants::ColorOrder, EmojiSkinToneVariants::Tone, EmojiSkinToneVariants::Version])
    ...
    let query = Query::select()
        .columns([EmojiGenderVariants::Hexcode, EmojiGenderVariants::Emoji, EmojiGenderVariants::BaseHexcode, EmojiGenderVariants::Ordering, EmojiGenderVariants::ColorOrder, EmojiGenderVariants::Gender, EmojiGenderVariants::Version])
    ...
    // We need to map hexcodes to glyphs, because Emoji struct uses glyph as key for annotations sometimes?
    // Actually Emoji::from_raw uses annotations_map keyed by emoji string (glyph).
    // But DB tables use hexcode.
    // So I need a map from hexcode to glyph.
    ...
    // Map of hexcode -> Emoji (for variants to attach to)
    // Actually, we process base emojis first, put them in groups, and then attach variants.
    // But variants are attached to `Emoji` struct.
    // We can first create all Emoji objects (base and variants) and then link them.
    // Or we can create base emojis, and then when processing variants, we find the base emoji and add the variant.
    
    // Since we need to put emojis into groups/subgroups structure, it's better to keep track of them.
    ...
    // let find_group_idx = |name: &str| -> Option<usize> {
    ...
    // let find_subgroup_idx = |groups: &Vec<Group>, g_idx: usize, s_name: &str| -> Option<usize> {
    //     groups[g_idx].subgroups.iter().position(|s| s.name == s_name)
    // };
    ...
    for raw in db_emojis {
        ...
        let group_name = raw.group_id
            .and_then(|id| group_map.get(&id))
            .cloned()
            .unwrap_or_else(|| "component".to_string());
            
        let subgroup_name = raw.subgroup_id
            .and_then(|id| subgroup_map.get(&id))
            .cloned()
            .unwrap_or_else(|| "other-object".to_string());
        
        let codepoint: Vec<u32> = raw.hexcode.split('-').map(|s| u32::from_str_radix(s, 16).unwrap_or(0)).collect();
        ...
        let emoji = Emoji {
            ...
            name: "".to_string(), // Will be filled from annotations or raw? DbEmoji doesn't have name/label. Wait, check schema.
            variants: vec![],
            skin_tones: vec![],
            gender_variants: vec![],
        ...
        if let Some(g_idx) = groups.iter().position(|g| g.name == group_name) {
             if let Some(s_idx) = groups[g_idx].subgroups.iter().position(|s| s.name == subgroup_name) {
                 ...
                 emoji_locations.insert(raw.hexcode.clone(), (g_idx, s_idx, groups[g_idx].subgroups[s_idx].emojis.len() - 1));
             }
        }
    }
    ...
    for variant in skin_tones {
         if let Some(base_hex) = &variant.base_hexcode {
             if let Some((g_idx, s_idx, e_idx)) = emoji_locations.get(base_hex) {
                 ...
                 let codepoint: Vec<u32> = variant.hexcode.split('-').map(|s| u32::from_str_radix(s, 16).unwrap_or(0)).collect();
                 let tone_val = SkinTone::from_codepoint(&codepoint);
