...
use crate::{Annotation, Emoji, group_subgroup::*};
...
pub async fn vectorize_test_data(
...
) -> reqwest::Result<(String, f32, Vec<Group>, HashMap<u8, String>, HashMap<u8, String>)> {
	...
	let group_map: HashMap<u8, String> =
		messages.groups.into_iter().map(|g| (g.order, g.key)).collect();

	let subgroup_map: HashMap<u8, String> =
		messages.subgroups.into_iter().map(|s| (s.order, s.key)).collect();
